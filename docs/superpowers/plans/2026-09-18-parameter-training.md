...
skipped, and an empty result set reads as success — measured, exactly that
happened while this step was being written. So it asserts the source directory
exists *first*, before anything looks at the wheel, and it refuses to finish when
the source tree yielded no modules to compare.
